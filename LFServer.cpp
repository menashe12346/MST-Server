...
int main() {
    ...
    fd_set masterSet; // File descriptor set for select.

    // Create thread pool with desired number of threads
    ThreadPool pool(4); // Adjust number of threads as needed
    ...
    while (true) {
        ...
        if (select(FD_SETSIZE, &readSet, nullptr, nullptr, nullptr) == -1) { // Monitor sockets for activity
            cerr << "Error on select" << endl;
            return 1;
        }

        for (int i = 0; i < FD_SETSIZE; ++i) { // Iterates through the file descriptors.
            if (FD_ISSET(i, &readSet)) {
                if (i == serverSocket) {
                    ...
                    if (clientSocket != -1) {
                        ...
                        // Enqueue a task to process this client's command
                        pool.enqueue([clientSocket] {
                            ...
                            if (nbytes > 0) {
                                processCommand(clientSocket, string(buffer)); // Process the command from client
                            }
